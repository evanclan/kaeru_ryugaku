...
>(({ className, children, ...props }, ref) => (
    <AccordionPrimitive.Header className="flex">
        <AccordionPrimitive.Trigger
            ref={ref}
            className={cn(
                "flex flex-1 items-center justify-between py-5 text-left font-medium text-slate-800 transition-all hover:text-emerald-600 [&[data-state=open]>svg]:rotate-180 cursor-pointer",
                className
            )}
            {...props}
        >
